package com.wavecat.mivlgu.chat.tokenizer;
...
public class GPT2Tokenizer {
    ...
    private static final String VOCAB_FILE = "vocab.bpe";
    private static final String ENCODER_FILE = "encoder.json";

    public GPT2Tokenizer(AssetManager manager) {
        try {
            this.encoder = new JSONParser(new InputStreamReader(manager.open(ENCODER_FILE), StandardCharsets.UTF_8)).parseObject();

            Map<Object, String> decoderMap = new HashMap<>();
            for (Map.Entry<String, Object> entry : encoder.entrySet()) {
                decoderMap.put(entry.getValue(), entry.getKey());
            }

            this.decoder = decoderMap;

            ArrayList<String> bpeLines = new ArrayList<>();
            try (BufferedReader br = new BufferedReader(new InputStreamReader(manager.open(VOCAB_FILE)))) {
                for (String line; (line = br.readLine()) != null; ) {
                    bpeLines.add(line);
                }
            }

            for (int i = 0; i < bpeLines.size(); i++) {
                String[] pairs = bpeLines.get(i).split(" ");
    ...
    public List<Integer> encode(String text) {
        ...
        for (String token : unicodes) {
            for (String bpeToken : bpe(token).split(" ")) {
                Object t = encoder.get(bpeToken);
                if (t == null) {
                    bpeTokens.add(-1);
                    continue;
                }
                bpeTokens.add(((BigInteger) t).intValue());
            }
        }
    ...
    private String trySubstring(String text, int length) {
        if (length >= text.length()) return text;
        else return text.substring(0, length);
    }

    public String crop(String text, int limit) {
        Matcher matcher = pattern.matcher(text);
        List<String> unicodes = new ArrayList<>();
        ...
        for (String token : unicodes) {
            for (String bpeToken : bpe(token).split(" ")) {
                Object t = encoder.get(bpeToken);
                if (tokens++ >= limit)
                    return trySubstring(text, (int) symbols);
                if (bpeToken.contains("null") || t == null)
                    symbols += 0.5F;
                else {
                    symbols += bpeToken.length();
                }
            }
        }
